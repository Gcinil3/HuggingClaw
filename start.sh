...
# Build allowedUserIds array properly
ALLOWED_USER_IDS_JSON="[]"
if [ -n "$TELEGRAM_USER_ID" ]; then
  ALLOWED_USER_IDS_JSON="[\"$TELEGRAM_USER_ID\""
  if [ -n "$TELEGRAM_USER_IDS" ]; then
    # Add comma-separated additional IDs
    IFS=',' read -ra EXTRA_IDS <<< "$TELEGRAM_USER_IDS"
    for id in "${EXTRA_IDS[@]}"; do
      trimmed_id=$(echo "$id" | xargs)
      ALLOWED_USER_IDS_JSON="$ALLOWED_USER_IDS_JSON, \"$trimmed_id\""
    done
  fi
  ALLOWED_USER_IDS_JSON="$ALLOWED_USER_IDS_JSON]"
fi

# Determine boolean values for config
...
# Create minimal config - most settings passed via environment variables
cat > "$CONFIG_DIR/openclaw.json" << EOF
{
  "channels": {
    "telegram": {
...
export OPENCLAW_LLM_PROVIDER="${LLM_PROVIDER:-auto}"
export OPENCLAW_LLM_MODEL="$LLM_MODEL"
export OPENCLAW_LLM_API_KEY="${LLM_API_KEY:-}"
export OPENCLAW_LLM_BASE_URL="${OLLAMA_BASE_URL:-}"
